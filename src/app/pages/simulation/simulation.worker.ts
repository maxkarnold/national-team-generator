...
const draw = (pts: GroupTeam[][], nbrOfGroups: number, availableRegions: Region[], startTime: number, failed = false): GroupTeam[][] => {
    ...
    const candidateGroups = groups.filter(
      ...
      group => {
        ...
        const regions3 = availableRegions
          .filter(r => r.qualifiers.auto + r.qualifiers.extra > 8 && r.qualifiers.auto + r.qualifiers.extra < 14)
          .map(r => r.value);
...
const drawAndSort = (t: GroupTeam[][], h: string, nbrOfGroups: number, availableRegions: Region[], startTime: number) =>
  sortGroups(h)(draw(t, nbrOfGroups, availableRegions, startTime));

addEventListener('message', ({ data }) => {
  const response = { draw: drawAndSort(data.potTeams, data.host.name, data.teamsInPot, data.availableRegions, data.start) };
